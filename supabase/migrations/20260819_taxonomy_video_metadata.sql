...
-- Prevent duplicate video IDs at database level as well as in the UI.
create unique index if not exists videos_video_id_unique_idx on public.videos(video_id) where video_id is not null and video_id <> '';

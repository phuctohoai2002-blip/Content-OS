...
function tableHead(type){
  ...
  if(type==="keywords")return`${select}<th>Keyword</th><th>Keyword Category</th><th>Niche</th><th>Pillar</th><th>Topic</th><th>Platform / Status</th><th>Actions</th>`;
...
function rowHtml(r){
  ...
  if(c.table==="keywords")return`<tr>${checkbox(r.id)}<td><div class="source-title" data-tax-inline data-id="${esc(r.id)}" data-field="keyword">${esc(r.keyword||"—")}</div></td><td>${esc(r.category||"—")}</td><td>${esc(parentName("niches",r.niche_id))}</td><td>${esc(parentName("pillars",r.pillar_id))}</td><td>${esc(parentName("topics",r.topic_id))}</td><td>${esc(r.platform||"—")} / ${esc(r.status||"active")}</td>${actions(r.id)}</tr>`;

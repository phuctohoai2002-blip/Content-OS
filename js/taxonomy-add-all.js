...
function open(){
  ...
  modal.innerHTML=`<div class="library-edit-modal taxonomy-add-all-modal"><div class="modal-header"><div><h3>Add All Taxonomy</h3><p>Use one CSV file to add Niches, Pillars, Topics, Keywords and Tags in one import.</p></div><button class="modal-close" data-close>×</button></div><div class="taxonomy-unified-template"><strong>One unified CSV template</strong><small>Use <code>record_type</code> = niche, pillar, topic, keyword or tag for each row.</small><div class="taxonomy-unified-columns">${COLUMNS.map(c=>`<code>${c}</code>`).join(" ")}</div><button type="button" class="button secondary" data-template>Download CSV template</button></div><div class="taxonomy-unified-upload"><label class="button secondary">Choose CSV<input type="file" accept=".csv,text/csv" data-file hidden></label><span data-file-name>No file selected</span><span data-row-count></span></div><div class="add-form-message hidden" data-message></div><div class="modal-actions"><button type="button" class="button secondary" data-close>Cancel</button><button type="button" class="button primary" data-import disabled>Import All</button></div></div>`;
...
function downloadTemplate(){const sample=[
 ["niche","DIG","Digital Design","Design and creative tools","","","","","","","","","","","","","","active"],
 ["pillar","DIG","","","PS","Photoshop","Photoshop workflow","","","","","","","","","","","active"],
 ["topic","DIG","","","PS","","","MASK","Layer Masks","Masking techniques","","","","","","","","active"],
 ["keyword","DIG","","","PS","","","MASK","","","KW001","layer mask","Tutorial","mat thuop lop","Douyin","","","active"],
 ["tag","DIG","","","PS","","","MASK","","","","","","","","Tutorial","Beginner tutorial","active"]
 ];
 const csv=[COLUMNS.join(","),...sample.map(r=>r.map(csvCell).join(","))].join("\n")+"\n";download(csv,"taxonomy-all-template.csv");}

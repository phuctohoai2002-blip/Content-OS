...
document.addEventListener("DOMContentLoaded", () => {
    ...
    modal.addEventListener("click", event => {
        if (event.target === modal || event.target.closest("[data-close-add-modal]")) closeModal();
    });
    ...
    function bindModal(type) {
        ...
        if (type === "video") {
            ...
            creatorCode.addEventListener("input", () => {
                form.elements.download_path.value = "";
                form.elements.creator_status.textContent = "";
            });
        }
    ...
    async function resolveCreatorPath(form) {
        ...
        if (!code) return;
        const status = form.elements.creator_status;
    ...
    async function saveVideo(form) {
        const payload = {
            video_id: form.elements.video_id.value.trim(),
    ...
    async function saveCreator(form) {
        const nicheId = form.elements.niche_id.value || null;
        const payload = {
            creator_id: form.elements.creator_id.value.trim(),
            name: form.elements.name.value.trim(),
            platform: form.elements.platform.value.trim(),
            niche_id: nicheId,
            sub_niche: form.elements.sub_niche.value.trim() || null,
            profile_link: form.elements.profile_link.value.trim() || null,
            total_videos: Number(form.elements.total_videos.value || 0),
            download_path: form.elements.download_path.value.trim() || null
        };
        if (!payload.creator_id || !payload.name || !payload.platform) throw new Error("Creator ID, Creator Name and Platform are required.");
        await insertWithSchemaFallback("creators", payload, [
            ["creator_id", "name", "platform", "niche_id", "sub_niche", "profile_link", "total_videos", "download_path"],
            ["creator_code", "name", "platform", "niche_id", "sub_niche", "profile_link", "total_videos", "download_path"]
        ]);
    }
    ...
    function creatorMatches(row, code) {
        const normalized = code.toLowerCase();
        return [row.creator_id, row.creator_code, row.channel_id, row.channel_code, row.code].some(value => String(value ?? "").toLowerCase() === normalized);
    }

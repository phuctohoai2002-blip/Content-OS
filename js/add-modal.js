...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function validateVideoId(form) {
        ...
        try {
            ...
            if (error) { if (looksLikeUnknownColumn(error)) { box.textContent = ""; return false; } throw error; }
    ...
    async function resolveCreator(form) {
        ...
        try {
            ...
            const path = getDownloadPath(creator); form.elements.download_path.value = path || ""; status.textContent = path ? `✓ ${creator.name || "Creator"} found` : `✓ ${creator.name || "Creator"} found — no download path saved`;

...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    window.addEventListener("nicheChanged", () => {
        if (window.location.hash.replace("#", "") === "dashboard" || !window.location.hash) initDashboardWorkspace();
    });
});
...
async function initDashboardWorkspace() {
    ...
    try {
        ...
        if (failed !== -1) {
            const error = results[failed].error;
            throw new Error(`${labels[failed]}: ${error.message || error.code || "Supabase request failed"}`);
        }
        ...
        // Fetch names separately instead of using PostgREST nested relations.
        // This keeps the dashboard independent of relationship metadata exposed by PostgREST.
        ...
        renderRecentContent(recentResult.data || [], nicheMap, pillarMap);
        ...
        if (errorBox) {
            errorBox.innerHTML = `<div class="empty-state"><div class="empty-state-icon">⚠</div><strong>Dashboard could not load</strong><p>${escapeHtml(error.message || "Unknown Supabase error")}</p><small>Check the failing Supabase request shown above.</small></div>`;
...
async function loadNicheNames() {
    ...
    if (error) throw new Error(`niche names: ${error.message || error.code}`);
    return new Map((data||[]).map(niche => [niche.id, niche]));
}

async function loadPillarNames() {
    const { data, error } = await supabase.from("pillars").select("id,name");
    if (error) throw new Error(`pillar names: ${error.message || error.code}`);
    return new Map((data||[]).map(pillar => [pillar.id, pillar.name]));
}
...
function renderDashboardPillars(rows, nicheMap) {
    const body = document.getElementById("dashboardPillarsBody"); if (!body) return;
    if (!rows.length) { body.innerHTML='<tr><td colspan="5"><div class="empty-state compact"><strong>No published performance yet</strong><p>Publish some videos to see pillar performance here.</p></div></td></tr>'; return; }
    body.innerHTML=rows.map(row=>{const niche=nicheMap.get(row.niche_id);return `<tr><td>${escapeHtml(row.pillar_name||"—")}</td><td>${escapeHtml(niche?.name||"—")}</td><td>${formatNumber(row.video_count)}</td><td>${formatNumber(row.total_views)}</td><td>${formatNumber(row.total_followers)}</td></tr>`}).join("");
}

function renderRecentContent(rows, nicheMap, pillarMap) {
    const body=document.getElementById("dashboardRecentBody"); if(!body)return;
    if(!rows.length){body.innerHTML='<tr><td colspan="4"><div class="empty-state compact"><strong>No content yet</strong><p>Create your first content item from the Content module.</p></div></td></tr>';return;}
    body.innerHTML=rows.map(row=>{const niche=nicheMap.get(row.niche_id);const pillarName=pillarMap.get(row.pillar_id)||"—";return `<tr><td>${escapeHtml(row.title||"Untitled content")}</td><td>${escapeHtml(niche?.name||"—")}</td><td>${escapeHtml(pillarName)}</td><td><span class="badge">${escapeHtml(formatStage(row.stage))}</span></td></tr>`}).join("");
}

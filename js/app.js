...
function selectNiche(nicheCode) {
    ...
    if (dot) {
        ...
        dot.classList.add(nicheCode === "FAS" ? "fashion" : nicheCode === "GRM" ? "grooming" : nicheCode === "DIG" ? "digital" : "master");
    }
    if (changed) window.dispatchEvent(new CustomEvent("nicheChanged", { detail: { nicheCode, nicheId } }));
}
...
async function initDashboardWorkspace() {
    ...
    try {
        ...
        const [nichesResult, creatorsResult, sourcesResult, readyResult, pillarsResult, recentResult] = await Promise.all([
            nicheId ? Promise.resolve({ count: 1, error: null }) : supabase.from("niches").select("id", { count: "exact", head: true }).eq("status", "active"),
        ...
        renderDashboardPillars(pillarsResult.data || [], nicheMap); renderRecentContent(recentResult.data || [], nicheMap);
        ...
        if (errorBox) { errorBox.innerHTML = `<div class="empty-state"><div class="empty-state-icon">⚠</div><strong>Dashboard could not load</strong><p>${escapeHtml(error.message || "Unknown Supabase error")}</p><small>Check your Supabase connection and table/view permissions.</small></div>`; errorBox.classList.remove("hidden"); }
    }
}
window.initDashboardWorkspace = initDashboardWorkspace;
async function loadNicheNames() { const { data, error } = await supabase.from("niches").select("id,name,niche_code"); if (error) throw error; return new Map((data || []).map(niche => [niche.id, niche])); }
...
function renderRecentContent(rows, nicheMap) { const body = document.getElementById("dashboardRecentBody"); if (!body) return; if (!rows.length) { body.innerHTML = '<tr><td colspan="4"><div class="empty-state compact"><strong>No content yet</strong><p>Create your first content item from the Content module.</p></div></td></tr>'; return; } body.innerHTML = rows.map(row => { const niche = nicheMap.get(row.niche_id); const pillarName = row.pillars?.name || "—"; const stage = formatStage(row.stage); return `<tr><td>${escapeHtml(row.title || "Untitled content")}</td><td>${escapeHtml(niche?.name || "—")}</td><td>${escapeHtml(pillarName)}</td><td><span class="badge">${escapeHtml(stage)}</span></td></tr>`; }).join(""); }
